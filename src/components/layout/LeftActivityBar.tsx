...
export const LeftActivityBar: React.FC = () => {
    ...
    const IconBtn = ({icon: Icon, id, label, isBottom = false}: any) => (
        <button
            ...
            className={cn(
                ...
                // Only add margin-bottom if it's NOT the bottom button
                !isBottom && "mb-2",
    ...
    return (
        <div
            className="w-12 bg-[#050505] border-r border-zinc-800 flex flex-col items-center py-4 z-20 shrink-0 h-full">
            {/* Primary Features */}
            ...
            {/* Bottom Actions */}
            <IconBtn icon={Settings} id="settings" label="Settings" isBottom/>
        </div>
    );
};
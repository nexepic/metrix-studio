...
import {ZenInspector} from './ZenInspector';
...
export const GraphCanvas: React.FC = () => {
    ...
    useEffect(() => {
        ...
        cy.on('tap', (evt) => {
            if (evt.target === cy) setSelection(null, null);
        });
            ...
            if (detail.algorithm === 'pagerank') {
                ...
                cy.batch(() => {
                    cy.nodes().forEach(node => {
                        ...
                        node.animate({style: {'width': newSize, 'height': newSize}, duration: 500});
                    });
                });
            }
                ...
                cy.batch(() => {
                    cy.nodes().forEach(node => {
                        ...
                        if (degree > 0.3) {
                            ...
                            node.animate({
                                style: {'opacity': 0.3, 'border-width': 1, 'width': 16, 'height': 16},
                                duration: 500
                            });
                        }
                    });
    ...
    useEffect(() => {
        ...
        // Calculate center coordinates to initialize nodes at center instead of (0,0)
        const cx = cy.width() / 2;
        ...
        const elements: ElementDefinition[] = [
            ...graphData.nodes.map(n => ({
                ...
                // Initialize position at center to prevent "corner flash"
                position: {x: cx, y: cyCenter}
            })),
        ...
        cy.batch(() => {
            cy.elements().remove();
            // Add new elements
            const addedEles = cy.add(elements);
            // Immediately hide them so the user doesn't see the initial dense clump
            addedEles.style({opacity: 0});
        });
        ...
        if (elements.length > 0) {
            try {
                ...
                // Smoothly fade in the elements while the layout is unfolding
                cy.elements().animate({
                    style: {opacity: 1},
                    duration: 600,
    ...
    const handleZoomIn = () => cyRef.current?.zoom({
        level: cyRef.current.zoom() + 0.2,
        renderedPosition: getCenter(cyRef.current)
    });

    const handleZoomOut = () => cyRef.current?.zoom({
        level: cyRef.current.zoom() - 0.2,
        renderedPosition: getCenter(cyRef.current)
    });

    // Modified: Use direct fit() instead of animate() for instant adjustment
    const handleFit = () => {
        if (!cyRef.current) return;
    ...
    return (
        <div
            className={`
                ...
                ${isFullscreen ? 'fixed inset-0 z-[100] w-screen h-screen' : 'relative w-full h-full'}
            `}
        >
            {/* Floating Inspector (Zen Mode) - Only show when fullscreen and something is selected */}
            {isFullscreen && <ZenInspector/>}
            ...
            {stats.nodes === 0 && isGraphVisible && (
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    ...
                    <p className="text-xs text-zinc-500 mt-2 max-w-[200px] text-center">Run a Cypher query to populate
                        the view.</p>
                </div>
            )}
            ...
            <div
                ...
                `}
            >
                <GraphBtn onClick={toggleFullscreen} icon={isFullscreen ? <Minimize2 size={16}/> : <Expand size={16}/>}
                          title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"} active={isFullscreen}/>
                ...
                <div className="h-[1px] bg-zinc-800 my-1"/>
                <GraphBtn onClick={() => setIsSearchOpen(!isSearchOpen)} icon={<SearchIcon size={16}/>}
                          title="Search Nodes" active={isSearchOpen}/>
            </div>
            ...
            {isSearchOpen && isGraphVisible && (
                <div className="absolute top-4 right-16 z-20 animate-in slide-in-from-right-4 fade-in duration-200">
                    <form onSubmit={handleSearch}
                        ...
                        <input autoFocus type="text" placeholder="Find ID or Label..." value={searchTerm}
            ...
            {stats.nodes > 0 && isGraphVisible && (
                <div
                    ...
                    <div
                        className="bg-zinc-950/80 backdrop-blur-md border border-zinc-800/80 rounded-lg px-3 py-1.5 text-[10px] shadow-xl flex items-center gap-3 text-zinc-400">
                        <div className="flex items-center gap-1.5">
                            <div className="w-2 h-2 rounded-full bg-indigo-500"/>
                            <span className="font-mono font-semibold text-zinc-100">{stats.nodes}</span> Nodes
                        </div>
                        <div className="h-3 w-[1px] bg-zinc-800"/>
                        <div className="flex items-center gap-1.5"><span
                            className="font-mono font-semibold text-zinc-100">{stats.edges}</span> Edges
                        </div>
                    </div>
                </div>
            )}
...
const GraphBtn: React.FC<GraphBtnProps> = ({onClick, icon, title, active}) => (
    <button onClick={onClick} title={title}
            className={`p-2 text-zinc-400 border rounded-md backdrop-blur-sm transition-all shadow-lg active:scale-95 ${active ? 'bg-indigo-500/20 border-indigo-500/50 text-indigo-300' : 'bg-zinc-900/80 border-zinc-800 hover:bg-zinc-800 hover:text-indigo-400'}`}>
        {icon}
    </button>
);
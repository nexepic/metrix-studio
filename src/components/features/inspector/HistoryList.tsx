        ...
        <ScrollArea className="h-full">
            <div className="flex flex-col">
                {history.map((item) => (
                    <div
                        key={item.id}
                        className="group flex flex-col gap-2 p-3 border-b border-zinc-800/50 hover:bg-zinc-900/60 transition-colors"
                    >
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button
                                    ...
                                    className="opacity-0 group-hover:opacity-100 p-1 hover:bg-indigo-500/20 hover:text-indigo-300 text-zinc-500 rounded transition-all"
                                    title="Load Query"
                                >
                                    <Play size={10}/>
                                </button>
                        ...
                        {/* Query Text: zinc-300 (Primary readability) */}
                        <code className={cn(
                            "text-[11px] font-mono break-all line-clamp-3 leading-relaxed",
                            item.status === 'error' ? "text-red-300" : "text-zinc-300"
                        )}>
                            {item.query}
                        </code>
                    </div>

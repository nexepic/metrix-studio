...
import { FolderOpen, Database, ChevronRight, Plus, ArrowLeft, Clock, Trash2, Search } from "lucide-react";
...
export const WelcomeScreen = () => {
    const { connectDatabase, createNewDatabase, recentFiles, removeRecentFile } = useApp();
    ...
    const safeConnect = async (path: string) => {
        try {
            ...
            const msg = typeof e === 'string' ? e : (e?.message || "Failed to connect.");
            alert(`Database Error:\n${msg}\n\nThis project has been removed from your history.`);
        }
    };
    ...
    const handleRemoveRecent = (path: string, e: React.MouseEvent) => {
        e.stopPropagation();
        removeRecentFile(path); // This function now handles AppStore state AND LocalStorage
    };

    // --- SUB-VIEW: Home ---
    const HomeView = () => (
        <div className="flex flex-col items-center gap-12 w-full max-w-[840px] animate-in fade-in zoom-in duration-500">
            {/* Header */}
            <div className="flex flex-col items-center gap-6">
                <div className="p-5 bg-zinc-900/50 rounded-3xl border border-zinc-800/50 shadow-2xl backdrop-blur-xl">
                    <img src={metrixIcon} alt="Logo" className="w-14 h-14 md:w-16 md:h-16 opacity-95 drop-shadow-lg"/>
                </div>
            ...
            <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-6">
                <ClassicActionCard
                    title="Open Existing"
                    desc="Select a local database folder or drag & drop"
            ...
            {recentFiles.length > 0 && (
                ...
                >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[2px]">Recent Connections ({recentFiles.length})</span>
    ...
    const HistoryView = () => {
        ...
        return (
            <div className="flex flex-col w-full max-w-[700px] h-[500px] bg-[#0c0c0e]/80 border border-zinc-800 rounded-3xl backdrop-blur-xl animate-in slide-in-from-bottom-4 duration-500 overflow-hidden shadow-2xl">
                {/* History Header */}
                <div className="p-6 border-b border-zinc-800/50 flex items-center justify-between bg-zinc-900/30">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setView('home')} className="p-2 hover:bg-zinc-800 rounded-xl text-zinc-400 hover:text-white transition-all">
                            <ArrowLeft size={20} />
                        </button>
                        <h2 className="text-lg font-bold text-white tracking-tight">Recent Projects</h2>
                    </div>
                    <div className="relative flex items-center group">
                        <Search className="absolute left-3 text-zinc-600 group-focus-within:text-indigo-500 transition-colors" size={14} />
                        <input
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-2">
                    {filtered.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-zinc-700">
                            ...
                            <p className="text-xs uppercase font-bold tracking-widest opacity-50">Empty</p>
                        </div>
                    ) : (
                        filtered.map((path) => (
                            <div
                            ...
                            >
                                <div className="p-3 bg-zinc-950 rounded-xl text-zinc-600 group-hover:text-indigo-400 transition-colors shadow-inner">
                                    <Database size={18}/>
                                </div>
                                <div className="flex flex-col flex-1 overflow-hidden">
                                    ...
                                    <span className="text-[10px] text-zinc-400 truncate font-mono opacity-60 mt-0.5 tracking-tight">
                                        {path}
                                    </span>
                                </div>
    ...
    return (
        <div className="h-full w-full bg-[#09090b] text-zinc-100 relative overflow-hidden select-none font-sans flex flex-col">
            ...
            <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] bg-emerald-900/5 rounded-full blur-[130px] pointer-events-none"/>

            <div className="flex-1 flex flex-col items-center justify-center overflow-hidden w-full p-8">
                {view === 'home' ? <HomeView /> : <HistoryView />}
            </div>

            <CreateDbModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onCreate={createNewDatabase} />

            {/* Footer with improved contrast links */}
            <div className="h-20 shrink-0 flex flex-col items-center justify-center gap-3 pb-4 bg-gradient-to-t from-[#09090b] via-[#09090b] to-transparent z-20 w-full border-t border-zinc-800/20">
                <div className="flex gap-10 text-[10px] font-black text-zinc-500 uppercase tracking-[2px]">
                    <button onClick={() => handleLink(APP_CONFIG.links.docs)} className="hover:text-white transition-colors">Documentation</button>
                    <button onClick={() => handleLink(APP_CONFIG.links.github)} className="hover:text-white transition-colors">GitHub</button>
                </div>
...
const ClassicActionCard = ({ title, desc, icon, onClick, theme }: any) => {
    ...
    return (
        ...
        >
            <div className="bg-[#0c0c0e]/95 rounded-[15px] p-8 flex flex-col items-center gap-5 border border-zinc-800/60 relative overflow-hidden h-full backdrop-blur-sm">
                <div className={cn("absolute inset-0 transition-opacity duration-500 opacity-0", hover && "opacity-100", theme === 'indigo' ? 'bg-indigo-500/5' : 'bg-emerald-500/5')} />
